...
[How it works](docs/COMO-FUNCIONA.md) · [Concepts](docs/RIVERS-CONCEITOS.md) · [Reports](calib/)
...
The model that estimates the shop's capacity for the day:

### 1. Shop capacity — conditional mean vs. schedule
...
Cross-checking the shop's data against what RIVERS suggested, bike by bike: **what RIVERS suggested** × **what the shop actually did** (recorded in Maestro) × **the real outcome** (did the bike exceed 3h?).

> The ruler that matters: I measured the time **until the bike is ready**, not until the customer picks it up — because a customer holding a loaner is in no hurry to return, which used to distort the count.
...
- [ ] **Queue logic** — to estimate how many bikes are waiting pre-diagnosis

## Stack

**Next.js 16** · **React 19** · **TypeScript** · **Tailwind 4**. Data via **ClickHouse** (peerdb replicas, live read). Decision log in **Supabase** (Postgres). Calibration and analysis in **Node** + **Python**.
